...
int main()
{
    ...
    for (int N = N_START; N <= N_STOP; N *= 2)
    {
        ...
        for (int m = 0; m < N_RUN; m++)
        {
            ...
            for (int i = 0; i < N; i += 4)
            {
                vst1q_s32(&c_vector[i], vmlaq_s32(vld1q_s32(&c_vector[i]), vld1q_s32(&a[i]), vld1q_s32(&b[i])));
            }
            ...
            {
                vst1q_s32(&c_vector_unroll4[i], vmlaq_s32(vld1q_s32(&c_vector_unroll4[i]), vld1q_s32(&a[i]), vld1q_s32(&b[i])));
                vst1q_s32(&c_vector_unroll4[i + 4], vmlaq_s32(vld1q_s32(&c_vector_unroll4[i + 4]), vld1q_s32(&a[i + 4]), vld1q_s32(&b[i + 4])));
                vst1q_s32(&c_vector_unroll4[i + 8], vmlaq_s32(vld1q_s32(&c_vector_unroll4[i + 8]), vld1q_s32(&a[i + 8]), vld1q_s32(&b[i + 8])));
                vst1q_s32(&c_vector_unroll4[i + 12], vmlaq_s32(vld1q_s32(&c_vector_unroll4[i + 12]), vld1q_s32(&a[i + 12]), vld1q_s32(&b[i + 12])));
            }
